...
### 1. `--cwd` and `--env` on `run` / `start`
Set working directory and environment natively instead of wrapping in
`sh -c 'cd DIR && K=V exec cmd'`, which reintroduces a shell and the escaping
surface 0.2.0 removed.
- Wire: add `cwd: Option<String>`, `env: Vec<(String,String)>` to `Request::Run`
  / `Request::Start`.
- rxd: in the grandchild, `chdir(cwd)` then `execvpe` with the merged env
  (inherit + overrides) before exec. No shell.
- CLI: `--cwd DIR`, repeatable `--env K=V`.

### 2. `rx wait HOST ID [--timeout SECS]`
Block server-side until a backgrounded/`start`ed process exits or the timeout
hits — completes the no-polling story for the async path.
- Reuses `run`'s wait-or-timeout loop against an existing process dir.
- Returns `completed` (with output, like `run`) or `running` on timeout.
- Wire: `Request::Wait { id, timeout_ms }`.

### 3. `rx cp LOCAL HOST:REMOTE [--mode MODE] [--owner USER] [--group GRP]`
